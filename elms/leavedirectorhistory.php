...
<html lang="en">
    <head>
        ...
        <!-- Title -->
        <title>DIRECTOR | Leave History </title>
...
<thead>
                                    ...
                                    <?php 
$eid=intval($_GET['empid']);
$sql = "SELECT LeaveType,ToDate,FromDate,Description,PostingDate,AdminRemarkDate,AdminRemark,Status,noofdays from tblleaves where empid=:eid";
$query = $dbh -> prepare($sql);
                                            ...
                                            <td><?php echo htmlentities($result->PostingDate);?></td>
                                             ...
                                             if($stats==1 && strtotime($result->ToDate) >= strtotime($now)){
                                              ...
                                              <span style="color: blue">waiting for approval</span>
												...
												<?php } if($stats==1 && strtotime($result->ToDate) < strtotime($now))  { ?>
												 <span style="color: darkgreen">Used</span>
												  <?php } if($stats==4 && strtotime($result->ToDate) < strtotime($now))  { ?>
												 <span style="color: limegreen">Partially Used</span>
												<?php } if($stats==0 || $stats==5 || $stats==2 || $stats==3){
